...
function injectWorkerRuntimeModule(webPackWithAlphaTab) {
  class AlphaTabWorkerRuntimeModule extends webPackWithAlphaTab.webpack.RuntimeModule {
    constructor() {
      super(
        'alphaTab audio worker chunk loading',
        webPackWithAlphaTab.webpack.RuntimeModule.STAGE_BASIC
      )
    }
    generate() {
      ...
      for (const c of this.chunk.getAllInitialChunks()) {
        if (
          webPackWithAlphaTab.webpack.javascript.JavascriptModulesPlugin.chunkHasJs(
            c,
            this.chunkGraph
          )
        ) {
 ...
 * @returns
 */
function configureAudioWorklet(
  webPackWithAlphaTab,
  pluginName,
  options,
  compiler,
...
function configureWebWorker(
  ...
  new webPackWithAlphaTab.webpack.javascript.EnableChunkLoadingPlugin('import-scripts').apply(
    compiler
  )
...
function injectWebWorkerDependency(webPackWithAlphaTab) {
  class AlphaTabWebWorkerDependency extends webPackWithAlphaTab.webpack.dependencies
    .ModuleDependency {
  ...
  AlphaTabWebWorkerDependency.Template = class WorkerDependencyTemplate extends (
    ...
    apply(dependency, source, templateContext) {
      ...
      source.replace(
        dep.range[0],
        dep.range[1] - 1,
        `/* worker import */ ${workerImportBaseUrl} + ${webPackWithAlphaTab.webpack.RuntimeGlobals.getChunkScriptFilename}(${JSON.stringify(chunk.id)}), ${webPackWithAlphaTab.webpack.RuntimeGlobals.baseURI}`
      )
    }
  }
  makeDependencySerializable(
    webPackWithAlphaTab,
    AlphaTabWebWorkerDependency,
    'AlphaTabWebWorkerDependency'
  )
  webPackWithAlphaTab.alphaTab.createWebWorkerDependency = (request, range, publicPath) =>
  ...
  webPackWithAlphaTab.alphaTab.registerWebWorkerDependency = (compilation, normalModuleFactory) => {
    ...
    compilation.dependencyTemplates.set(
      AlphaTabWebWorkerDependency,
      new AlphaTabWebWorkerDependency.Template()
    )
  }
}
...
function injectWorkletRuntimeModule(webPackWithAlphaTab) {
  class AlphaTabWorkletStartRuntimeModule extends webPackWithAlphaTab.webpack.RuntimeModule {
    constructor() {
      super(
        'alphaTab audio worklet chunk lookup',
        webPackWithAlphaTab.webpack.RuntimeModule.STAGE_BASIC
      )
    }
    generate() {
...
function injectWorkletDependency(webPackWithAlphaTab) {
   ...
   */
  class AlphaTabWorkletDependency extends webPackWithAlphaTab.webpack.dependencies
    .ModuleDependency {
  ...
  webPackWithAlphaTab.alphaTab.createWorkletDependency = (request, range, publicPath) =>
...
const absoluteToRequest = (context, maybeAbsolutePath) => {
  if (maybeAbsolutePath[0] === '/') {
    ...
    let resource =
      querySplitPos === -1 ? maybeAbsolutePath : maybeAbsolutePath.slice(0, querySplitPos)
  ...
  configureAssetCopy(webPackWithAlphaTab, pluginName, compiler, compilation) {
    ...
    compilation.hooks.processAssets.tapAsync(
      ...
      async (_, callback) => {
        ...
        if (alphaTabSourceDir) {
          try {
            await fs__namespace.promises.access(
              path__namespace.join(alphaTabSourceDir, 'alphaTab.mjs'),
          ...
          await Promise.all(
            files
              ...
              .map(async (file) => {
                ...
                await fs__namespace.promises.copyFile(
                  sourceFilename,
                  path__namespace.join(outputPath, subdir, file.name)
                )
