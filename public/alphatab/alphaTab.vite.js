...
const relativeUrlMechanisms = {
  ...
  es: (relativePath) =>
    getResolveUrl(`'${escapeId(partialEncodeURIPath(relativePath))}', import.meta.url`),
  iife: (relativePath) => getRelativeUrlFromDocument(relativePath),
  // NOTE: make sure rollup generate `module` params
  system: (relativePath) =>
    getResolveUrl(`'${escapeId(partialEncodeURIPath(relativePath))}', module.meta.url`),
  umd: (relativePath) =>
...
function createToImportMetaURLBasedRelativeRuntime(format, isWorker) {
  ...
  return (filename, importer) => ({
    runtime: toRelativePath(
      path__namespace.posix.relative(path__namespace.dirname(importer), filename)
    ),
  })
}
...
function createDepsOptimizer(config) {
  ...
  const metadata = parseDepsOptimizerMetadata(
    fs__namespace.readFileSync(path__namespace.join(depsCacheDirPrefix, METADATA_FILENAME), 'utf8'),
    depsCacheDirPrefix
  )
    ...
    async buildStart() {
      ...
      if (!alphaTabSourceDir) {
        try {
          ...
          alphaTabSourceDir = path__namespace.join(
            resolvedConfig.root,
...
function workerPlugin(options) {
  ...
  return {
    ...
    async transform(raw, id) {
      ...
      const envScriptPath = JSON.stringify(
        path__namespace.posix.join(resolvedConfig.base, ENV_PUBLIC_PATH)
      )
    ...
    generateBundle(_, bundle) {
      ...
      for (const asset of workerMap.assets.values()) {
        ...
        if (duplicateAsset) {
          const content =
            duplicateAsset.type === 'asset' ? duplicateAsset.source : duplicateAsset.code
